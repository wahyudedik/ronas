...
<div class="grid grid-cols-1 gap-6">
    ...
    <div>
        ...
        <input type="text" name="heading" value="{{ old('heading', $history->heading ?? '') }}"
            class="w-full rounded-md border-gray-300" />
    </div>
    <div>
        <label class="block text-sm font-medium mb-1">Description</label>
        <textarea id="description" name="description" rows="4" class="w-full rounded-md border-gray-300">{{ old('description', $history->description ?? '') }}</textarea>
    </div>
    <div>
    ...
    <div>
        ...
        <input type="number" name="sort_order" value="{{ old('sort_order', $history->sort_order ?? 0) }}"
            class="w-full rounded-md border-gray-300" />
    </div>
    <div class="flex items-center gap-2">
        <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300"
            {{ old('is_active', $history->is_active ?? false) ? 'checked' : '' }} />
