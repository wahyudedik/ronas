<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.admissions.request-programs.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-semibold">
                Add Program
            </a>
        </div>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                            ...
                            <tbody id="request-programs-sortable" class="divide-y divide-gray-100 dark:divide-gray-700">
                                @foreach ($programs as $program)
                                    <tr data-id="{{ $program->id }}">
                                        ...
                                        <td class="py-2">
                                            <form action="{{ route('admin.admissions.request-programs.toggle', $program) }}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                        ...
                                        <td class="py-2 text-right">
                                            <a href="{{ route('admin.admissions.request-programs.edit', $program) }}" class="text-indigo-600 hover:underline">Edit</a>
                                            <form action="{{ route('admin.admissions.request-programs.destroy', $program) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                    ...
                    <div class="mt-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <button id="request-programs-undo-btn" type="button" class="hidden rounded-md border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                            Undo last reorder
                        </button>
                    </div>

                    <form id="request-programs-order-form" method="POST" action="{{ route('admin.admissions.request-programs.reorder') }}">
                        @csrf
                        <input type="hidden" name="order" id="request-programs-order-input">
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        (function () {
            const tbody = document.getElementById('request-programs-sortable');
            const input = document.getElementById('request-programs-order-input');
            const form = document.getElementById('request-programs-order-form');
            ...
            const history = [];
            const undoBtn = document.getElementById('request-programs-undo-btn');
