<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.facilities.items.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-semibold">
                Add Item
            </a>
        </div>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                            ...
                            <tbody id="items-sortable" class="divide-y divide-gray-100 dark:divide-gray-700">
                                @foreach ($items as $item)
                                    <tr data-id="{{ $item->id }}">
                                        ...
                                        <td class="py-2">
                                            <form action="{{ route('admin.facilities.items.toggle', $item) }}" method="POST">
                                                @csrf
                                                @method('PATCH')
                                        ...
                                        <td class="py-2 text-right">
                                            <a href="{{ route('admin.facilities.items.edit', $item) }}" class="text-indigo-600 hover:underline">Edit</a>
                                            <form action="{{ route('admin.facilities.items.destroy', $item) }}" method="POST" class="inline">
                                                @csrf
                                                @method('DELETE')
                    ...
                    <div class="mt-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
                        <button id="items-undo-btn" type="button" class="hidden rounded-md border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
                            Undo last reorder
                        </button>
                    </div>

                    <form id="items-order-form" method="POST" action="{{ route('admin.facilities.items.reorder') }}">
                        @csrf
                        <input type="hidden" name="order" id="items-order-input">
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        (function () {
            const tbody = document.getElementById('items-sortable');
            const input = document.getElementById('items-order-input');
            const form = document.getElementById('items-order-form');
            ...
            const history = [];
            const undoBtn = document.getElementById('items-undo-btn');
