<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.landing.news.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-semibold">
                Add News
            </a>
        </div>
    </x-slot>
            ...
            <div class="bg-white dark:bg-gray-800 shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                        ...
                        <tbody id="news-sortable" class="divide-y divide-gray-100 dark:divide-gray-700">
                            @foreach ($news as $item)
                                <tr data-id="{{ $item->id }}">
                                    ...
                                    <td class="py-2">
                                        <form action="{{ route('admin.landing.news.toggle', $item) }}" method="POST">
                                            @csrf
                                            @method('PATCH')
                                    ...
                                    <td class="py-2 text-right">
                                        <a href="{{ route('admin.landing.news.edit', $item) }}" class="text-indigo-600 hover:underline">Edit</a>
                                        <form action="{{ route('admin.landing.news.destroy', $item) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="ml-3 text-red-600 hover:underline" onclick="return confirm('Delete this news item?')">Delete</button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
    ...
    <form id="news-order-form" method="POST" action="{{ route('admin.landing.news.reorder') }}">
        @csrf
        <input type="hidden" name="order" id="news-order-input">
    </form>

    <div class="mt-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <button id="news-undo-btn" type="button" class="hidden rounded-md border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
            Undo last reorder
        </button>
    </div>
    ...
    <script>
        (function () {
            ...
            const input = document.getElementById('news-order-input');
            const form = document.getElementById('news-order-form');
        ...
        const history = [];
        const undoBtn = document.getElementById('news-undo-btn');
    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.landing.news.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md text-sm">Add News</a>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <form method="POST" action="{{ route('admin.landing.section.update', 'news') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        @csrf
                        @method('PUT')
                        <div>
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                            ...
                            <tbody class="divide-y divide-gray-100 dark:divide-gray-700">
                                @forelse ($newsItems as $newsItem)
                                    <tr>
                                        <td class="py-2">{{ $newsItem->title }}</td>
                                        <td class="py-2">{{ $newsItem->category }}</td>
                                        <td class="py-2">{{ optional($newsItem->published_at)->format('Y-m-d') }}</td>
                                        <td class="py-2">{{ $newsItem->sort_order }}</td>
                                        <td class="py-2">{{ $newsItem->is_active ? 'Yes' : 'No' }}</td>
                                        <td class="py-2 text-right">
                                            <a href="{{ route('admin.landing.news.edit', $newsItem) }}" class="text-indigo-600 hover:underline dark:text-indigo-400">Edit</a>
                                            <form method="POST" action="{{ route('admin.landing.news.destroy', $newsItem) }}" class="inline">
                                                @csrf
                                                @method('DELETE')
