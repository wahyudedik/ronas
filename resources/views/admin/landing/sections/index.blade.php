<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            ...
            <a href="{{ route('admin.landing.sections.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-semibold">
                Add Section
            </a>
        </div>
    </x-slot>
                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700 text-sm">
                            ...
                            <tbody id="sections-sortable" class="divide-y divide-gray-100 dark:divide-gray-700">
                                @foreach ($sections as $section)
                                    <tr data-id="{{ $section->id }}">
                                        <td class="py-2">
                                            @include('admin.landing.partials.drag-handle')
                                        </td>
                                        <td class="py-2">{{ $section->key }}</td>
                                        ...
                                        <td class="py-2 text-right">
                                            <a href="{{ route('admin.landing.sections.edit', $section) }}" class="text-indigo-600 hover:underline">Edit</a>
    ...
    <div class="mt-4 max-w-7xl mx-auto sm:px-6 lg:px-8">
        <button id="sections-undo-btn" type="button" class="hidden rounded-md border border-slate-300 px-3 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50">
            Undo last reorder
        </button>
    </div>

                    <form id="sections-order-form" method="POST" action="{{ route('admin.landing.sections.reorder') }}">
                        @csrf
                        <input type="hidden" name="order" id="sections-order-input">
                    </form>
                </div>
            </div>
        </div>
    ...
    <script>
        (function () {
            ...
            const input = document.getElementById('sections-order-input');
            const form = document.getElementById('sections-order-form');
            ...
            const history = [];
            const undoBtn = document.getElementById('sections-undo-btn');
