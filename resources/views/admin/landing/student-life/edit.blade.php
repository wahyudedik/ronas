        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('admin.landing.student-life.update', $item) }}"
                        ...
                        <div>
                            ...
                            <x-text-input id="title" name="title" class="mt-1 block w-full" :value="old('title', $item->title)"
                                required />
                        ...
                        <div>
                            ...
                            <select id="category" name="category"
                                ...
                                <option value="organizations"
                                    {{ old('category', $item->category) == 'organizations' ? 'selected' : '' }}>
                                    Organizations</option>
                                <option value="athletics"
                                    {{ old('category', $item->category) == 'athletics' ? 'selected' : '' }}>Athletics
                                </option>
                                <option value="facilities"
                                    {{ old('category', $item->category) == 'facilities' ? 'selected' : '' }}>Facilities
                                </option>
                                <option value="support_services"
                                    {{ old('category', $item->category) == 'support_services' ? 'selected' : '' }}>
                                    Support Services</option>
                                <option value="gallery"
                                    {{ old('category', $item->category) == 'gallery' ? 'selected' : '' }}>Gallery
                                </option>
                            </select>
                        ...
                        <div>
                            ...
                            <x-text-input id="badge_text" name="badge_text" class="mt-1 block w-full" :value="old('badge_text', $item->badge_text)"
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description"
                                class="mt-1 block w-full rounded-md border-gray-300 dark:border-gray-700 dark:bg-gray-900">{{ old('description', $item->description) }}</textarea>
                        ...
                        <div>
                            ...
                            @if (!empty($item->image))
                                <img src="{{ asset(ltrim($item->image, '/')) }}" alt="Student life image"
                        ...
                        <div>
                            ...
                            <x-text-input id="link_label" name="link_label" class="mt-1 block w-full"
                                :value="old('link_label', $item->link_label)" />
                        </div>
