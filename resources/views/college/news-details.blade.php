                    ...
                    <div class="article-header">
                        @if ($news->category)
                            <div class="meta-categories" data-aos="fade-up">
                                <a href="{{ route('college.news') }}?category={{ $news->category->slug }}"
                        ...
                        <div class="article-meta" data-aos="fade-up" data-aos-delay="200">
                            <div class="author">
                                @if ($news->author_image)
                                    <img src="{{ asset(ltrim($news->author_image, '/')) }}"
                                        alt="{{ $news->author->name ?? 'Admin' }}" class="author-img">
                                @else
                                    <img src="/College/assets/img/person/person-m-6.webp" alt="Author" class="author-img">
                                @endif
                                <div class="author-info">
                                    <h4>{{ $news->author->name ?? 'Admin' }}</h4>
                                </div>
                            </div>
                            <div class="post-info">
                                @if ($news->published_at)
                                    <span><i class="bi bi-calendar4-week"></i>
                                        {{ $news->published_at->format('F d, Y') }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="article-footer" data-aos="fade-up">
                        ...
                        @if ($news->category)
                            <div class="article-tags">
                                ...
                                <div class="tags">
                                    <a href="{{ route('college.news') }}?category={{ $news->category->slug }}"
                                        class="tag">{{ $news->category->name }}</a>
                                </div>
                            </div>
                        @endif
                    </div>
                                ...
                                <div class="col-md-4">
                                    <article>
                                        <div class="post-img">
                                            @if ($item->image)
                                                <img src="{{ asset(ltrim($item->image, '/')) }}" alt="{{ $item->title }}"
                                                    class="img-fluid">
                                            @else
                                                <img src="/College/assets/img/blog/blog-post-1.webp" alt=""
                                                    class="img-fluid">
                                            @endif
                                        </div>
                                        @if ($item->category)
                                        ...
                                        @if ($item->published_at)
                                            <p class="post-date">
                                                <time
                                                    datetime="{{ $item->published_at->format('Y-m-d') }}">{{ $item->published_at->format('M d, Y') }}</time>
                                            </p>
                                        @endif
                                    </article>
                                </div>
