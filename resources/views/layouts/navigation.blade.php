<nav x-data="{ open: false }" class="bg-white dark:bg-gray-800 border-b border-gray-100 dark:border-gray-700">
        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    @endcan
                    @can('manage landing')
                        <x-nav-link :href="route('admin.landing.index')" :active="request()->routeIs('admin.landing.*')">
                            {{ __('Landing Setup') }}
                        </x-nav-link>
                    @endcan
                    @can('manage about')
                    ...
                    @endcan
                    <x-nav-link :href="route('admin.alumni.stories.index')" :active="request()->routeIs('admin.alumni.*')">
                        {{ __('Alumni') }}
                    </x-nav-link>
                    <x-nav-link :href="route('admin.contact.messages.index')" :active="request()->routeIs('admin.contact.*')">
                        {{ __('Contact') }}
                    </x-nav-link>
                    @can('manage legal pages')
                        <x-nav-link :href="route('admin.legal.pages.index')" :active="request()->routeIs('admin.legal.*')">
                            {{ __('Legal') }}
                        </x-nav-link>
                    @endcan
                </div>
    ...
    <div :class="{ 'block': open, 'hidden': !open }" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            ...
            @endcan
            <x-responsive-nav-link :href="route('admin.alumni.stories.index')" :active="request()->routeIs('admin.alumni.*')">
                {{ __('Alumni') }}
            </x-responsive-nav-link>
            <x-responsive-nav-link :href="route('admin.contact.messages.index')" :active="request()->routeIs('admin.contact.*')">
                {{ __('Contact') }}
            </x-responsive-nav-link>
            @can('manage legal pages')
                <x-responsive-nav-link :href="route('admin.legal.pages.index')" :active="request()->routeIs('admin.legal.*')">
                    {{ __('Legal') }}
                </x-responsive-nav-link>
            @endcan
        </div>
